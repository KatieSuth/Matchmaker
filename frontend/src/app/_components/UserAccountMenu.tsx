...
// Discord avatar menu: My Account and Logout. Hidden when signed out.
import { useState, useRef, useEffect, useCallback } from "react";
import Image from "next/image";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { useAuth } from "@/app/_context/AuthContext";
import { discordAvatarUrl } from "@/app/_lib/constants";
...
export default function UserAccountMenu() {
  ...
  return (
    <div className="relative ml-auto flex-shrink-0" ref={dropdownRef}>
        ...
        <span className="text-sm font-semibold tracking-[0.04em] max-w-[120px] whitespace-nowrap overflow-hidden text-ellipsis max-sm:hidden">
          {user
            ? user.display_name?.trim() ||
              (user.discord_name?.trim() ? `@${user.discord_name.trim()}` : "Account")
            : "Account"}
        </span>
      ...
      {dropdownOpen && (
        <div
          className="animate-drop-in dropdown-panel absolute top-[calc(100%+10px)] right-0 min-w-[180px] rounded-lg overflow-hidden z-[100]"
          role="menu"
        >
