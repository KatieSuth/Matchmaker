...
func TestValidateAuth(t *testing.T) {
	...
	_, numStatus := validateAuth(jwtSecret, "")
	...
	testutil.WithTestTx(t, func(q *db.Queries) {
		...
		_, numStatus := validateAuth(jwtSecret, accessToken)
		if numStatus != http.StatusUnauthorized {
		...
		_, numStatus = validateAuth([]byte{}, fmt.Sprintf("Bearer %s", accessToken))
		if numStatus != http.StatusUnauthorized {
		...
		userId, numStatus := validateAuth(jwtSecret, fmt.Sprintf("Bearer %s", accessToken))
		...
		_, numStatus = validateAuth(jwtSecret, fmt.Sprintf("Bearer %s", accessToken))
		if numStatus != http.StatusUnauthorized {
